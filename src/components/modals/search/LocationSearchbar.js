...
import { StyleSheet, View, ScrollView, Animated, FlatList, Keyboard, Text, Button, Dimensions, Easing, TouchableOpacity } from 'react-native';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';
import colors from 'assets/variables/colors';
import ListSelectionItem from './ListSelectionItem';
import WsSearchbar from './WsSearchbar';
import { onLocationSearchbarChanged, onLocationSearchbarFocused, onLocationSearchbarPressed, closeSearchbar, onCoordinatesChanged, onMarkersDisplayed } from 'actions/map-reducer-action';
    ...
    renderSearchLocationContainer = () => {
        return this.props.isSearchbarFocus &&
            (<Animated.View enabled style={{ position: 'absolute', top: 105, backgroundColor: colors.white, zIndex: 99, width: '100%' }}>
                <Animated.ScrollView style={{ paddingHorizontal: 10, height: this.state.animation }}
...
const mapDispatchToProps = dispatch => {
    return bindActionCreators({ onLocationSearchbarChanged, onLocationSearchbarFocused, onLocationSearchbarPressed, closeSearchbar, onCoordinatesChanged, onMarkersDisplayed }, dispatch);
}
